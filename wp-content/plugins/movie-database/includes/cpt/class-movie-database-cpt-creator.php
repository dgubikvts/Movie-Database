...
require_once MVDB_PLUGIN . 'includes/cpt/class-movie-database-taxonomy-creator.php';

class MVDB_CPT_Creator
{
    private string $name;
    ...
    public MVDB_Taxonomy_Creator $taxonomy;

    public function __construct()
    {
        $this->taxonomy = new MVDB_Taxonomy_Creator();
    }
    ...
    public function register(): void
    {
        ...
        register_post_type($args['name'], array_diff_key($args, ['name']));
    }
    ...
    public function register_taxonomy(string $slug, string $singular): void
    {
        $this->taxonomy->setSlug($slug)->setCPT($this->name)->setLabels([
            'name'          => sprintf(_x('%s', 'Taxonomy name', 'movie-database'), ucfirst($slug)),
            'singular_name' => sprintf(_x('%s', 'Taxonomy singular name', 'movie-database'), ucfirst($singular)),
        ]);
    }
}